...
import com.example.demo.entity.SocketUserInfo;
...
public class webSocketController {
    ...
    //静态属性
    private static IVMessageService ivMessageService;
    //通过set注入
    @Autowired
    public void setChatService(IVMessageService message) {
        ivMessageService = message;
    }
    ...
    public void onOpen(Session session, @PathParam(value="role") Integer role) {
        ...
        if (role.equals(1)) {
            ...
            if (findFreeServer() != null){
                ...
                start.put("msg","尊敬的用户，您好！请问有什么可以为您效劳！");
                sendMsg(userInfo.getSession(), JSON.toJSONString(start));
            } else {
                //告诉用户系统繁忙
    ...
    public void onMessage(String message, Session session) {
        ...
        if (serverInfo != null) {
            ...
            System.out.println("用户"+ session.getId()+"发送消息：\""+ message +"\"给客服"+userSessionMap.get(session.getId()).getTargetSessionId());
            ...
            if (null != userSessionMap.get(session.getId()).getTargetSessionId()){
                ...
                vMessage.setSeek(message);
                vMessage.setSeekTime(new Date());
                ivMessageService.save(vMessage);
            } else{//同上
                sendMsg(session,JSON.toJSONString(result));
            }
        }
